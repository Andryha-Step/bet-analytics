					...
					{forecast.events.map((command, index) => {
						return (
							<ListItemView key={command.id} style={{ borderBottomWidth: Number(index < forecast.events.length - 1) }}>
								<ListItemHeader>
									<ListItemCommand>
										<ListItemCommandIcon source={{ uri: command.team_1_logo }} />
										<ListItemCommandText>{command.team_1_name}</ListItemCommandText>
									</ListItemCommand>
									<ListItemCommand>
										<ListItemCommandText>{command.team_2_name}</ListItemCommandText>
										<ListItemCommandIcon source={{ uri: command.team_2_logo }} />
									</ListItemCommand>
								</ListItemHeader>
								<ListItemSportName>ФУТБОЛ</ListItemSportName>
							</ListItemView>
						)
					})}
				</ListView>
...
const ListItemCommandText = styled.Text`
	...
	margin: 0 8px;
`
const ListItemCommandIcon = styled.Image`
	width: 16px;
	height: 16px;
`
